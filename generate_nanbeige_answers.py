...
DEFAULT_BASE_URL = "https://lqp982d0pidrdtza.us-east-1.aws.endpoints.huggingface.cloud/v1/"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate one Nanbeige4.1-3B answer for each prompt in a txt file."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Sampling temperature. Nanbeige model card recommends 0.6.",
    )
    parser.add_argument(
        "--top-p",
        type=float,
        default=DEFAULT_TOP_P,
        help="Top-p sampling value. Nanbeige model card recommends 0.95.",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
...
def derive_output_paths(input_path: Path) -> tuple[Path, Path, Path]:
    ...
    return (
        parent / f"{stem}_nanbeige_messages.jsonl",
        parent / f"{stem}_nanbeige_answers.txt",
        parent / f"{stem}_nanbeige_partial.jsonl",
    )
